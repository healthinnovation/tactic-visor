...
ccpp <- read_csv(ccpp_filepath) %>% 
  filter(nomcp %in% c("QUISTOCOCHA", "LLANCHAMA", "EL VARILLAL", "SAN LUCAS", "12 DE ABRIL")) %>%
...
areas <- st_read(areas_filepath, quiet = TRUE) %>%
  filter(village %in% c("QUISTOCOCHA", "LLANCHAMA", "EL VARILLAL", "SAN LUCAS", "12 DE ABRIL"))
...
drone_1km <- st_read(drone_1km_filepath, quiet = TRUE) %>% 
  ...
  filter(!village %in% c("SAN MARTIN II ZONA","ZUNGAROCOCHA"),
         village %in% c("QUISTOCOCHA", "LLANCHAMA", "EL VARILLAL", "SAN LUCAS", "12 DE ABRIL"))
...
map_layers <- function() {
  ...
  map <- leaflet() %>% 
    ...
    setView(lng = -73.44775,lat = -4.0,zoom = 9) %>%
    addPulseMarkers(
      data = ccpp,
